...
#%% face
# layer 1
with tf.name_scope('Conv_F1'):
  conv_f1_kernel = tf.get_variable('Conv_F1_Filter',[11,11,channel,96],initializer=init)
  conv_f1_wx = tf.nn.conv2d(face, conv_f1_kernel, [1, 4, 4, 1], padding='VALID')
  conv_f1_b = tf.Variable(tf.zeros([96]))
  relu_f1 = tf.nn.relu(tf.nn.bias_add(conv_f1_wx, conv_f1_b), name='ReLU_F1')
  pool_f1 = tf.nn.max_pool(relu_f1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID', name='Pool_F1')
  norm_f1 = tf.nn.lrn(pool_f1,5,alpha=0.0001, beta=0.75, name='Norm_F1')

# layer 2
with tf.name_scope('Conv_F2'):
  conv_f2_kernel = tf.get_variable('Conv_F2_Filter',[5,5,96,256],initializer=init)
  conv_f2_wx = tf.nn.conv2d(norm_f1, conv_f2_kernel, [1, 1, 1, 1], padding='SAME')
  conv_f2_b = tf.Variable(tf.zeros([256]))
  relu_f2 = tf.nn.relu(tf.nn.bias_add(conv_f2_wx,conv_f2_b), name='ReLU_F2')
  pool_f2 = tf.nn.max_pool(relu_f2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='VALID', name='Pool_F2')
  norm_f2 = tf.nn.lrn(pool_f2,5,alpha=0.0001, beta=0.75, name='Norm_F2')

# layer 3
with tf.name_scope('Conv_F3'):
  conv_f3_kernel = tf.get_variable('Conv_F3_Filter',[3,3,256,384],initializer=init)
  conv_f3_wx = tf.nn.conv2d(norm_f2, conv_f3_kernel, [1, 1, 1, 1], padding='SAME')
  conv_f3_b = tf.Variable(tf.zeros([384]))
  relu_f3 = tf.nn.relu(tf.nn.bias_add(conv_f3_wx,conv_f3_b), name='ReLU_F3')

# layer 4
with tf.name_scope('Conv_F4'):
  conv_f4_kernel = tf.get_variable('Conv_F4_Filter',[1,1,384,64],initializer=init)
  conv_f4_wx = tf.nn.conv2d(relu_f3, conv_f4_kernel, [1, 1, 1, 1], padding='VALID')
  conv_f4_b = tf.Variable(tf.zeros([64]))
  relu_f4 = tf.nn.relu(tf.nn.bias_add(conv_f4_wx,conv_f4_b), name='ReLU_F4')
  
#%% FC face
with tf.name_scope('FC_F1'):
  fc_f1_x = tf.reshape(relu_f4,[batch,-1])
  fc_f1_w = tf.get_variable('FC_F1_W',[np.shape(fc_f1_x)[1],128],initializer=init)
  fc_f1_b = tf.Variable(tf.ones([128]))
  fc_f1 = tf.nn.bias_add(tf.matmul(fc_f1_x,fc_f1_w),fc_f1_b)
  relu_fc_f1 = tf.nn.relu(fc_f1,name='ReLU_FC_F1')
  drop_fc_f1 = tf.nn.dropout(relu_fc_f1,1.,name='Dropout_FC_F1')
  
with tf.name_scope('FC_F2'):
  fc_f2_w = tf.get_variable('FC_F2_W',[np.shape(drop_fc_f1)[1],64],initializer=init)
  fc_f2_b = tf.Variable(tf.ones([64]))
  fc_f2 = tf.nn.bias_add(tf.matmul(drop_fc_f1,fc_f2_w),fc_f2_b)
  relu_fc_f2 = tf.nn.relu(fc_f2,name='ReLU_FC_F2')
  drop_fc_f2 = tf.nn.dropout(relu_fc_f2,1.,name='Drop_FC_F2')
...
with tf.name_scope('FC1'):
  fc1_x = tf.concat([drop_fc_f2,drop_fc_fg2],1)
...
generate_plots(history[0:i_epoch+1],i_epoch+1)
